...
void add_to_cart(string enetered_car)
{
    ...
    for(int i = 0; i <= total_cars; i++)
    {
        if(entered_engine_power == engines[i])
        {
            ...
            // recording cart cars engines to create a unique way to access each car's details in the cart
            cart_engines[cars_in_cart] = engines[i];
            cars_in_cart++;
            // breaking out of the loop so that the car_engine_exists will not become false in the next iteration
            break;
        }
    }
...
void take_customer_info()
{
    string user_name, account_number, pin, number;
